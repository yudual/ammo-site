...
let spritesGroup: THREE.Group | null = null
...
// 动态生成高清晰悬浮 Sprite 标签 (仅对选中/悬浮/核心重点单位生成)
function createBadgeSprite(item: GeoItem, isSelected = false): THREE.Sprite {
  ...
  const bgColor = isSelected ? '#eb7c65' : isUni ? 'rgba(37, 99, 235, 0.92)' : 'rgba(190, 85, 67, 0.92)'
  ...
  sprite.userData = { item, baseLat: item.lat, baseLng: item.lng }
  return sprite
}
...
function initScene() {
  ...
  const sphereMat = new THREE.MeshPhongMaterial({
    color: isDark ? 0x0a0e0c : 0xf0f5f2,
    emissive: isDark ? 0x040605 : 0xd6ded9,
    specular: isDark ? 0x1f2e26 : 0xffffff,
    shininess: 25,
    transparent: true,
    opacity: 0.98,
  })
  ...
  // 2. 菲涅尔全息大气漫射光晕
  const atmosGeo = new THREE.SphereGeometry(GLOBE_RADIUS * 1.04, 32, 32)
  ...
  const gridMat = new THREE.LineBasicMaterial({
    color: isDark ? 0x22332a : 0xc2d2ca,
    transparent: true,
    opacity: 0.22,
  })
...
// 更新精致 3D 地标 (解决遮挡拥挤问题：精细化光柱 + 呼吸光环 + 仅对选中/悬浮/重点单位悬浮微标签)
function updatePins() {
  if (!pinsGroup || !spritesGroup) return
  while (pinsGroup.children.length > 0) pinsGroup.remove(pinsGroup.children[0])
  while (spritesGroup.children.length > 0) spritesGroup.remove(spritesGroup.children[0])

  // 重点示范高校名单（即便远景也优雅显示）
  const anchorIds = new Set(['uni-bit', 'uni-njust', 'uni-nuc', 'uni-nwpu', 'uni-sylu', 'uni-aust'])

  props.items.forEach((item) => {
    const isSelected = props.selectedItem?.id === item.id
    const isHovered = hoveredItem.value?.id === item.id
    const isUni = item.type === 'university'
    const colorHex = isSelected ? 0xffffff : isUni ? 0x3b82f6 : 0xeb7c65
    const pos = latLngToVector3(item.lat, item.lng, GLOBE_RADIUS)

    // 1. 精细激光引线柱
    const pillarHeight = isSelected ? 10 : isUni ? 5 : 3.5
    const pillarGeo = new THREE.CylinderGeometry(0.35, 0.1, pillarHeight, 6)
    ...
    // 2. 地面扩散呼吸光环 (半径微调，杜绝巨型重叠)
    const ringGeo = new THREE.RingGeometry(0.6, 1.3, 16)
    ...
    // 3. 悬浮文字标牌：仅对选中、悬浮、或少数骨干高校渲染，彻底解决“全部遮挡地球”的问题！
    if (isSelected || isHovered || (anchorIds.has(item.id) && currentZoom > 200) || currentZoom < 160) {
      const sprite = createBadgeSprite(item, isSelected)
      sprite.position.copy(pos.clone().multiplyScalar(1 + (pillarHeight + 2.5) / GLOBE_RADIUS))
...
function onPointerMove(e: MouseEvent | TouchEvent) {
  ...
  if (isPointerDown && globeGroup) {
    ...
    previousPointerPos = { x: clientX, y: clientY }
  } else if (!isPointerDown && canvasContainer.value && camera && pinsGroup) {
    // 鼠标悬停拾取
    ...
    if (intersects.length > 0 && intersects[0].object.userData?.item) {
      if (hoveredItem.value?.id !== intersects[0].object.userData.item.id) {
        hoveredItem.value = intersects[0].object.userData.item
        updatePins()
      }
...
function animate() {
  ...
  if (globeGroup) {
    if (isFlying) {
      ...
      // 动量阻尼减速
      angularVelocity.x *= 0.93
      angularVelocity.y *= 0.93

      // 根据用户的自转开关设定决定是否维持极轻微自转
      if (props.autoRotate && Math.abs(angularVelocity.y) < 0.0001) {
        angularVelocity.y = 0.0008
      }
    ...
    // 剔除地球背面标牌：检查每个 Sprite 是否在摄像机可见半球
    if (camera && spritesGroup) {
      const cameraDir = camera.position.clone().normalize()
      spritesGroup.children.forEach((sprite) => {
        const worldPos = sprite.position.clone().applyMatrix4(globeGroup!.matrixWorld)
        const dot = worldPos.normalize().dot(cameraDir)
  ...
  >
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-x-4 gap-y-0.5 mt-1 border-t pt-1.5 border-[var(--border)]">
        ...
        <span>自转模式: <strong :class="autoRotate ? 'text-[var(--status-positive)]' : 'text-[var(--text-tertiary)]'">{{ autoRotate ? '动态巡航' : '定点锁定' }}</strong></span>
      </div>
    </div>
